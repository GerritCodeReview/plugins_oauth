...
public class ScribeOAuthClientTest {
  ...
  public void exchangeCode_withCodeVerifier_passesPkceVerifier() throws Exception {
    ...
    new ScribeOAuthClient(service, false, true)
        .exchangeCode(new OAuthVerifier("code"), "code-verifier");
    ...
    verify(service, never()).getAccessToken(anyString());
  }

  @Test
  public void exchangeCode_pkceDisabled_ignoresCodeVerifier() throws Exception {
    when(service.getAccessToken(anyString())).thenReturn(accessToken("at", "Bearer", "raw"));

    // PKCE is off for this client, so even a (stale/foreign) session verifier must not be
    // forwarded: fall back to the plain token request, never the PKCE one.
    new ScribeOAuthClient(service).exchangeCode(new OAuthVerifier("code"), "stale-verifier");

    verify(service).getAccessToken("code");
    verify(service, never()).getAccessToken(any(AccessTokenRequestParams.class));
  }
