...
class InitOAuth implements InitStep {
  ...
  InitOAuth(ConsoleUI ui, Section.Factory sections, @PluginName String pluginName) {
    ...
    this.office365OAuthProviderSection =
        sections.get(PLUGIN_SECTION, pluginName + AzureActiveDirectory.CONFIG_SUFFIX_LEGACY);
    this.azureActiveDirectoryAuthProviderSection =
        sections.get(PLUGIN_SECTION, pluginName + AzureActiveDirectory.CONFIG_SUFFIX);
  ...
  public void run() throws Exception {
    ...
    // E-> No, we either are setting up on an new system or using the new azure conifg
    else {
