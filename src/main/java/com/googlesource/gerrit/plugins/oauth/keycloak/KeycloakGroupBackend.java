...
public class KeycloakGroupBackend implements GroupBackend {
  private static final String KEYCLOAK_UUID_PREFIX = "keycloak:";
  ...
  private final String keycloakExternalIdScheme;
  private final KeycloakGroupCache keycloakGroupCache;

  @Inject
  public KeycloakGroupBackend(KeycloakOAuthService keycloakOAuthService, KeycloakGroupCache keycloakGroupCache) {
    this.keycloakOAuthService = keycloakOAuthService;
    this.keycloakExternalIdScheme = this.keycloakOAuthService.getExternalIdScheme();
  ...
  public GroupDescription.Basic get(AccountGroup.UUID uuid) {
    ...
    return new GroupDescription.Basic() {
      ...
      public String getName() {
        return KEYCLOAK_NAME + groupName;
      }
  ...
  public Collection<GroupReference> suggest(String name, ProjectState project) {
    ...
    } else if (name.startsWith(KEYCLOAK_NAME)) {
      return suggestKeycloak(name.substring(KEYCLOAK_NAME.length()));
    } else {
  ...
  public GroupMembership membershipsOf(CurrentUser user) {
    ...
    Set<String> allExtIds = user.asIdentifiedUser()
                              .getExternalIdKeys()
                              .stream()
                              .map(Object::toString)
                              .collect(Collectors.toSet());
    if (log.isDebugEnabled()) {
      log.debug("User {} has external IDs: {}", user.getUserName(), allExtIds);
    }
    if (allExtIds.isEmpty()) {
      return GroupMembership.EMPTY;
    }
    Optional<ExternalId.Key> id = user.asIdentifiedUser()
                      .getExternalIdKeys()
                      .stream()
                      .filter(e -> isKeycloakExtIdScheme(e))
                      .findAny();
    if (!id.isPresent()) {
      if (log.isDebugEnabled()) {
        log.debug("No Keycloak-scheme externalId found for user {}", user.getUserName());
      }
      return GroupMembership.EMPTY;
    }
    String extId = id.get().toString();
    if (log.isDebugEnabled()) {
  ...
  private boolean isKeycloakExtIdScheme(ExternalId.Key id) {
    return id.isScheme(keycloakExternalIdScheme);
  }
}
