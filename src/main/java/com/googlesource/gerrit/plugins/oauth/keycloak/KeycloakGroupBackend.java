...
public class KeycloakGroupBackend implements GroupBackend {
  ...
  @Inject
  public KeycloakGroupBackend(
      KeycloakOAuthService keycloakOAuthService, KeycloakGroupCache keycloakGroupCache) {
    this.keycloakExternalIdScheme = keycloakOAuthService.getExternalIdScheme();
  ...
  public GroupMembership membershipsOf(CurrentUser user) {
    ...
    Optional<ExternalId.Key> id =
        user.asIdentifiedUser().getExternalIdKeys().stream()
            .filter(e -> e.isScheme(keycloakExternalIdScheme))
            .findAny();
    if (!id.isPresent()) {
    ...
    Set<AccountGroup.UUID> groupUuids =
        keycloakGroupCache.get(extId).stream()
            .map(group -> AccountGroup.uuid(KEYCLOAK_UUID_PREFIX + group))
            .collect(Collectors.toSet());
    if (log.isDebugEnabled()) {
      log.debug(
          "User {} mapped to {} group UUIDs: {}",
          user.getUserName(),
          groupUuids.size(),
          groupUuids);
    }
  ...
  private Set<GroupReference> suggestKeycloak(String partialName) {
    ...
    for (String groupName : keycloakGroupCache.getAllGroups()) {
