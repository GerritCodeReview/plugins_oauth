...
class HttpModule extends ServletModule {
  ...
  protected void configureServlets() {
    ...
    cfg = cfgFactory.getFromGerritConfig(pluginName + AzureActiveDirectory.CONFIG_SUFFIX);
    if (cfg.getString(InitOAuth.CLIENT_ID) != null) {
      bind(OAuthServiceProvider.class)
          .annotatedWith(Exports.named(AzureActiveDirectory.CONFIG_SUFFIX))
          .to(AzureActiveDirectory.class);
    }

    cfg = cfgFactory.getFromGerritConfig(pluginName + AzureActiveDirectory.CONFIG_SUFFIX_LEGACY);
    if (cfg.getString(InitOAuth.CLIENT_ID) != null) {
      bind(OAuthServiceProvider.class)
          .annotatedWith(Exports.named(AzureActiveDirectory.CONFIG_SUFFIX))
          .to(AzureActiveDirectory.class);
